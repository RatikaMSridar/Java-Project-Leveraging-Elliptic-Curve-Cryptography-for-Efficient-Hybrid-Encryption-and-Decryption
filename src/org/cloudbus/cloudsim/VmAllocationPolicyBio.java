...
//import org.cloudbus.cloudsim.examples.CS2_RoundRobin;
 ...
 */
public class VmAllocationPolicyBio extends VmAllocationPolicy {
	 ...
	 * @post $none
	 */
	public VmAllocationPolicyBio(List<? extends Host> list) {
		super(list);

		//consigo recuperar freePes de outros datacenters?
		//Resp.: aparentemente nao porque essa lista estah 
		//processando os hosts do objeto corrente
		...
		for (Host host : getHostList()) {
			...
			//Para cada datacenter, recupera a lista de hosts
...
/*			//Exibe CPUs livre nos hosts do datacenter
  			System.out.println(freePes);			
			try {
				Thread t = new Thread();
				t.sleep(1000);
			} catch (Exception e){}
*/			
		...
		if (!getVmTable().containsKey(vm.getUid())) { //if this vm was not created
			...
			do {//we still trying until we find a host or until we try all of them
				...
				if (idx!=-1){
					try {				
						...
						if (result) { //if vm were succesfully created in the host
							//Log.printLine("PolicyBio VM: "+vm.getId()+ " host: "+vm.getHost().getId() + " datacenter: "+vm.getHost().getDatacenter().getId());
							...
							String texto = "PolicyBio VM: "+vm.getId()+ " host: "+vm.getHost().getId()+" datacenter: "+vm.getHost().getDatacenter().getId() + "\n";
							//String texto = host.getId()+"\n";							
				...
				//enquanto nao varrer todos os hosts disponiveis, na tentativa de alocar a VM 
...
");
	...
	public float calcularFitness(int host, List<Integer> freePesTmp){
		...
		float custoLocalidade=0;
		float fitness=0;

		//Alfa eh uma constante superior ao limite maximo de bandwidth
		//para que o peso de uma CPU livre seja superior ao da bandwidth
		float alfa=1000;

		//Beta eh uma constante para a bandwidth
		float beta=1;

		try{		
			//Calcula a largura de banda do host ate o recurso
			//com a intencao de realizar a alocacao proxima ao recurso
			//host-1 pq a lista de hosts comeca em 0		
			custoLocalidade = Float.parseFloat(d.get(nodes[host-1]).toString());
			//System.out.println(custoBandwidth);

			//Verifica se existe CPUs disponiveis no Selected Host
			//host-1 pq a lista de hosts comeca em 0
			fitness = (alfa*freePesTmp.get(host-1)) + (beta*custoLocalidade) + 1; //+1 para nao ter fitness 0 
			...
			System.out.println("Excecao: Verifique se as cardinalidades da topologia Brite\n" +
			"e da variavel DATACENTER_HOSTS sao as mesmas.");
			System.exit(0);
		}
		return fitness;
	...
	public int avaliar(List<Integer> freePesTmp){
		...
		while (i<TAM_POPULACAO && achou==false){
			...
			//
			//Ordena os servidores no cromossomo, com base no fitness
			ordenarCromoLocal(i,freePesTmp);

			int j=0;
			int moreFree = Integer.MIN_VALUE;
			idx = -1;			
			//Varre o cromossomo em busca do servidor com mais cpus disponiveis
			while (j<TAM_CROMO-1){

				//Recupera o host no cromossomo
				host=Integer.parseInt(R2[i][j])-1;//-1 pq a lista de hosts comeca em 0		

				//Dentre os hosts do cromossomo, seleciona 
				//aquele com a maior qtde de cpus livres
				//
				//we want the host with less pes in use
	...
	public void recoverTopology(){
		...
		try { 
			...
			//BufferedReader fileBrite = new BufferedReader(new FileReader(f));
			//build the topology
			//new BRITETopology_v2(filePath,0);
			//close the file
			//fileBrite.close();
