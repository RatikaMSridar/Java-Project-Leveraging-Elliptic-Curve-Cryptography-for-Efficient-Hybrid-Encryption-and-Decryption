 ...
 */
public class DatacenterBrokerBio extends SimEntity {
	 ...
	 * @post $none
	 */
	public DatacenterBrokerBio(String name) throws Exception {
		super(name);
	...
	public void ordenar(float [][] custoLocalidade){

		for (int pass=1; pass<custoLocalidade.length; pass++) //passagens
			for (int i=0; i<custoLocalidade.length-1; i++){//uma passagem
				//Ordena com base no custo de bandwidth ao recurso
				if (custoLocalidade[i][1] > custoLocalidade[i+1][1])
					troca(custoLocalidade,i,i+1);

			}//fim for

	}//fim ordenar

	public void troca(float [][] custoLocalidade, int i1, int i2){

		float [] aux=new float[custoLocalidade[0].length];

		aux[0]=custoLocalidade[i1][0];
		aux[1]=custoLocalidade[i1][1];

		custoLocalidade[i1][0] = custoLocalidade[i2][0];
		custoLocalidade[i1][1] = custoLocalidade[i2][1];

		custoLocalidade[i2][0]=aux[0];
		custoLocalidade[i2][1]=aux[1];		
...
/*		//Exibir o custo de bandwidth do datacenter em relacao ao recurso
 		for (int i=1; i<nodes.length; i++){
			Util.MSG("Bandwidth cost to reach node[" + i + "] from node [0]: " + Float.parseFloat(d.get(nodes[i]).toString()));
		}//fim for		 	
		try {
...
*/				
		...
		//custoLocalidade = [datacenterID, custo]
		//Ex.: [1, 0.0]
		//     [2, 3.0]
		//     [3, 1.0] ...
		float [][] custoLocalidade = new float[numDataCenters][2];

		it = getDatacenterIdsList().iterator();
		int i=0;
		while (it.hasNext()){
			//Guarda os campos
			custoLocalidade[i][0]= Integer.parseInt(it.next().toString());
			custoLocalidade[i][1] = Float.parseFloat(d.get(nodes[i]).toString());
			System.out.println("Datacenter[" + custoLocalidade[i][0] + "]: Custo de banda em relacao ao recurso [0]: " + custoLocalidade[i][1]);
			i=i+1;
...
*/			
		...
		if (getVmsCreatedList().size() == getVmList().size() - getVmsDestroyed()) { // all the requested VMs have been created
			...
			if (getVmsRequested() == getVmsAcks()) { // all the acks received, but some VMs were not created
				...
				ordenar(custoLocalidade);								

				// find id of the next datacenter that has not been tried
				int nextDatacenterId=0;
				for (int j=0; j<custoLocalidade.length; j++){
					//Se o datacenter ainda nao foi requisitado
					nextDatacenterId=(int)custoLocalidade[j][0];
	...
	protected void submitCloudlets() {
		...
		for (Cloudlet cloudlet : getCloudletList()) {
			...
			Log.printLine(CloudSim.clock()+": "+getName()+ ": Sending cloudlet(data---) "+cloudlet.getCloudletId()+" to VM #"+vm.getId());
