...
//import org.cloudbus.cloudsim.examples.CS3_Bio;

/**
 * DatacentreBroker represents a broker
 * acting on behalf of a user. It hides VM management,
 * as vm creation, submission of cloudlets to this VMs
 ...
 */
public class DatacenterBrokerMINMIN extends SimEntity {
	...
	//Preciso ordenar os datacenters de acordo com a proximidade do recurso
	//
	//Primeiro, escolhe o datacenter. 
	//Depois, toda uma fila de processamento de VMs
	//ocorre at� esgotar os recursos desse datacenter.
	//Depois, um novo datacenter eh alocado.
	//
	//O padrao eh escolher um datacenter de forma incremental,
	//o que eh um desperdicio de recursos caso o custo de processamento (workload), largura de banda ou 
	//distancia em relacao ao recurso sejam elevados.
	//
	//Eh necessario um broker que aloque as VMs considerando
	//a distancia do datacenter em relacao ao recurso
	 ...
	 * @post $none
	 */
	public DatacenterBrokerMINMIN(String name) throws Exception {
		super(name);

		//Recupera a topologia de conexao entre os datacenters
		recoverTopology();
	...
	protected void processResourceCharacteristics(SimEvent ev) {
		...
		if (getDatacenterCharacteristicsList().size() == getDatacenterIdsList().size()) {
			setDatacenterRequestedIdsList(new ArrayList<Integer>());
			//Comeca a alocacao no primeiro datacenter da lista
			createVmsInDatacenter(getDatacenterIdsList().get(0));
			//createVmsInDatacenter(getDatacenterIdsList().get(2));
		}
	}
	...
	public void ordenar(double [][] custo){

		for (int pass=1; pass<custo.length; pass++) //passagens
			for (int i=0; i<custo.length-1; i++){//uma passagem
				//Ordena com base no custo de bandwidth ao recurso
				if (custo[i][1] > custo[i+1][1])
					troca(custo,i,i+1);
	...
	public void troca(double [][] custo, int i1, int i2){

		double [] aux=new double[custo[0].length];

		aux[0]=custo[i1][0];
		aux[1]=custo[i1][1];

		custo[i1][0] = custo[i2][0];
		custo[i1][1] = custo[i2][1];
	...
	protected void processVmCreate(SimEvent ev) {
		...
		//Ideia: O algoritmo MIN-MIN eh utilizado para o escalonamento de tarefas
		//       Estou interessado no escalonamento de VMs para servidores em datacenters.]
		//		 Por isso, elejo os datacenters com menor capacidade de processamento (menor custo) primeiro,
		//       com a intenção de aumentar gradativamente o custo de alocacao das VMs.
		
		//Preciso listar os datacenters disponiveis
		Iterator it = getDatacenterIdsList().iterator();
		int numDataCenters=0;
		while (it.hasNext()){
			numDataCenters=numDataCenters+1;

			//Estou assumindo um custo proporcional ao indice, ou seja,
			//os primeiros datacenters têm menor capacidade de processamento.			
			
			//Tenho o datacenterID
			//Recupero as informacoes dele
			DatacenterCharacteristics characteristics = getDatacenterCharacteristicsList().get(it.next());
			System.out.println(characteristics.getCostPerMem());
...
*/		
		...
		//custo = [datacenterID, custo]
		//Ex.: [1, 0.0]
		//     [2, 3.0]
		//     [3, 1.0] ...
		double [][] custo = new double[numDataCenters][2];
		it = getDatacenterIdsList().iterator();
		int i=0;		
		Object dataObj=null;
		...
		while (it.hasNext()){
			//Guarda os campos
			dataObj=it.next();			
			custo[i][0]=Integer.parseInt(dataObj.toString()); 
			characteristics = getDatacenterCharacteristicsList().get(dataObj);
			custo[i][1] = characteristics.getCostPerMem();
			System.out.println("Datacenter[" + custo[i][0] + "]: Custo de uso do datacenter: " + custo[i][1]);
			i=i+1;
		...
		if (getVmsCreatedList().size() == getVmList().size() - getVmsDestroyed()) { // all the requested VMs have been created
			...
			if (getVmsRequested() == getVmsAcks()) { // all the acks received, but some VMs were not created
				...
				//Ordena a lista de datacenters com base no menor custo 
				//de largura de banda em relacao ao recurso solicitado 
				//(no caso, simulacao para proximidade do recurso 0)

				//Busca o no com o menor custo de banda ao recurso 0
				//Ordenacao por referencia, ou seja, o vetor retornara ordenado
				ordenar(custo);								

/*				for (int j=0; j<custo.length; j++){
					System.out.println(custo[j][1]);
				}//fim for
				
				//Exibir o custo do datacenter  	
				try {
					Thread t = new Thread();
					t.sleep(1000000);
				} catch (Exception e){}
*/				
				...
				int nextDatacenterId=0;
				for (int j=0; j<custo.length; j++){
					//Se o datacenter ainda nao foi requisitado
					nextDatacenterId=(int)custo[j][0];
					if (!getDatacenterRequestedIdsList().contains(nextDatacenterId)) {
						//Nesse metodo, guarda o datacenter para os quais jah foram alocadas VMs 
	...
	protected void createVmsInDatacenter(int datacenterId) {
		...
		for (Vm vm : getVmList()) {
			//Processa as VMs restantes no proximo datacenter
		...
		//Guarda os datacenters que jah foram processados
