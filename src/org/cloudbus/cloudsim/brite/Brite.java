...
/*				//Esse trecho comentado funciona,
			 ...
			 *                
				...
				rgm.parse("src/org/cloudbus/cloudsim/brite/seed_file");

				//create our glorious model and give it a random gen manager
				Model m = ParseConfFile.Parse("src/org/cloudbus/cloudsim/brite/conf_files/ASImport_BRITE.conf");
...
/*				//Esse trecho comentado funciona,
	...
	private void runExecutable(String args) {
		...
		try {
			...
			if (runC)
				...
				runThis+=" src/org/cloudbus/cloudsim/brite/seed_file"; /*java version also requires a java seed*/
	...
	JButton logo = new JButton(new ImageIcon("src/org/cloudbus/cloudsim/brite/" + "logo_unicamp.jpg"));
	...
	{ 
		if (e.getSource().equals(ExportLocationBrowse)) {
			try {
				...
				Util.MSG("Nenhum arquivo selected: " + exc.getMessage());				
			}//fim catch
		}
	}
...
final class AboutPanel extends JDialog implements ActionListener {
	...
	public AboutPanel() {
		...
		String s= null;
		try {
			s="file:"+System.getProperty("user.dir") + "/src/org/cloudbus/cloudsim/brite/"+
			"about.html";
