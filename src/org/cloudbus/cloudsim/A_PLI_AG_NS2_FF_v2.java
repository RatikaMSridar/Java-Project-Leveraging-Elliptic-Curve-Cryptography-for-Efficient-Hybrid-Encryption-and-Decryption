...
public class A_PLI_AG_NS2_FF_v2 { 
	...
	public A_PLI_AG_NS2_FF_v2(){  
		...
		if (FF){
			...
			FF_DATACENTER_POINTER=NUM_DATACENTERS-1;
			while(NUM_ITERACAO<=FF_GROUP && FF_DATACENTER_POINTER>=0){
				...
				if (alocou){
					...
					if (FF_PORCENTAGEM_PERDA_TOTAL <= FF_LIMIAR_PERDA){
						...
						FF_SERVER_POINTER = ((FF_DATACENTER_POINTER+1)*NUM_SERVIDORES)-1;
	...
	public boolean FF_alocarGrupo(){
		...
		while(i<C.length && allocated){
			...
			while(FF_SERVER_POINTER>=0 && !allocated){
