...
public class MapReduceAG_old {
	...
	public MapReduceAG_old(){
		...
		perdaPacotesTotalVMs=realizarParserPerda();
		
		//Atualiza o fitness do cromossomo
		//fitness = valor da funcao objetivo
		int campoFitness;
		double fitness=0;
		campoFitness=(P2.length)-1;
		fitness = realizarParserFuncaoObjetivo();			
		//P2[i][campoFitness]=String.format("%1$,.2f",fitness);
	...
	public void atualizarTopologia(){
		...
		try
		{
			...
			//System.out.println("Processo["+RANK+"]: "+"numNodes: " + NUM_NODES);
			//Salta o campo Nodes
			linha=file.readLine();
			info.append(linha+"\n");
			...
			//System.out.println("Processo["+RANK+"]: "+"numEdges: " + NUM_EDGES);
			//Salta o campo Edges
			linha=file.readLine();
			info.append(linha+"\n");
			...
			//System.out.println("Processo["+RANK+"]: "+"numDatacenters: " + NUM_DATACENTERS);
			...
			//System.out.println("Processo["+RANK+"]: "+"numServers: " + NUM_SERVIDORES);
			...
			//System.out.println("Processo["+RANK+"]: "+"numVMs: " + NUM_VMS);
			...
			//System.out.println("Processo["+RANK+"]: "+"nodoDestino: " + NODO_DESTINO);	
			...
			System.out.println("Processo["+RANK+"]: "+"Excecao ao abrir o arquivo: " + ARQUIVO_TOPOLOGIA_DATACENTER+"_"+RANK + "\n" + e.getMessage());
		}//fim catch
	...
	public void gerarModeloLingo(){
		...
		try
		{
			...
			token.nextToken();
			int numNodes = Integer.parseInt(token.nextToken());
			System.out.println("Processo["+RANK+"]: "+"numNodes: " + numNodes);
			//Salta o campo Nodes
			file.readLine();
			...
			//Guarda todas as informacoes dos Edges
			//System.out.println("numEdges: " + numEdges);
			//EDGE[0][from, to, bw, type]
			//    [1][from, to, bw, type]
			//    ...
			String [][] EDGES_ARQUIVO = new String[numEdges][4];
			//Descricao dos campos
			linha = file.readLine();			
			String edgeFrom="";
			String edgeTo="";
			...
			for (int i=0; i<numEdges; i++){
				...
				//Quero agrupar os fluxos dos servidores				
				linha = file.readLine();
				token = new StringTokenizer(linha,"\t");
				//id
				token.nextToken();
				//from
				edgeFrom = token.nextToken();
				//to
				edgeTo = token.nextToken();
				//Length
				token.nextToken();
				//Delay
				token.nextToken();
				//Bandwidth
				edgeBW = token.nextToken();
				//ASfrom
				token.nextToken();
				//Asto
				token.nextToken();
				//Type
				edgeType=token.nextToken();
				//Other
				token.nextToken();

				//Guardo todas as arestas (irei precisar mais ah frente)
				EDGES_ARQUIVO[i][0]=edgeFrom;
				EDGES_ARQUIVO[i][1]=edgeTo;
				EDGES_ARQUIVO[i][2]=edgeBW;
				EDGES_ARQUIVO[i][3]=edgeType;
			...
			System.out.println("Processo["+RANK+"]: "+"numDatacenters: " + numDatacenters);
			//Salta o campo Datacenters
			file.readLine();
			...
			System.out.println("Processo["+RANK+"]: "+"numServers: " + numServers);
			...
			System.out.println("Processo["+RANK+"]: "+"numVMs: " + numVMs);
			...
			System.out.println("Processo["+RANK+"]: "+"nodoDestino: " + nodoDestino);
			...
			while (i<edges.length) {	
				...
				custosLinks.append("cr_" + edges[i].getSource() + "_" + edges[i].getDestination() + " <= " + edges[i].getBW() + ";\n");
				i++;
				System.out.print(i + " ");
			}//fim while
			...
			System.out.println("\nPasso16: Criacao do modelo LINGO");
	...
	public void realizarParserResultLingo(){
		...
		try
		{
				...
				while (linhaResult!=null){
					...
					if (t1.hasMoreTokens()){
						...
						if (matcher.find()){
							...
							if (valorAlocacao>0.5){ //Por conta de nao ser mais um modelo com variaveis binarias
			...
			try{
				...
				out.write("---\n");
				*/
				out.write("IndiceDatacenter IndiceServidor NumeroVMsAlocadasPorServidor ConsumoEnergia\n");
				i=0;
				...
				while (i<resultConsumoEnergiaRouter.length){
					if(resultConsumoEnergiaRouter[i]!=null)
						out.write(i + " " + resultConsumoEnergiaRouter[i] + "\n");
					else
						out.write(i + " " + 0 + "\n");
					i++;
				}//fim while
	...
	public void gerarModeloNS2(){	
		...
		try
		{
			...
			while (i<EDGES.length){
				...
				//n(0) -- n(1) -- n(111) -- ...
				if (!EDGES[i][3].equals("SERVER")){

					conteudoNS2.append("\nputs \"Link n("+EDGES[i][0]+")--n("+EDGES[i][1]+")"+"\";flush stdout;");
					//Com a adicao de roteadores de borda e servidores
					//eh necessario acrescentar nodos ah topologia original
					if (Integer.parseInt(EDGES[i][0])>=numNodosOriginal){
						conteudoNS2.append("\n   #IndiceNS2: n("+ indiceNodosNS2 +")");
						conteudoNS2.append("\n   set n("+EDGES[i][0]+") [$ns node]");						
						conteudoNS2.append("\n$n("+EDGES[i][0]+") color blue");
						conteudoNS2.append("\n$n("+EDGES[i][0]+") shape hexagon");
						indiceNodosNS2++;
					...
					if (EDGES[i][3].equals("BORDER_ROUTER"))
						//banda.append("BorderRouter_"+(indiceNodosNS2+1)+"--"+EDGES[i][1]+" "+EDGES[i][2]+"\n");
						banda.append("BorderRouter_"+i+"--"+EDGES[i][1]+" "+EDGES[i][2]+"\n");
					else
						banda.append(EDGES[i][0]+"--"+EDGES[i][1]+" "+EDGES[i][2]+"\n");					
					...
					if (t1.hasMoreTokens()){
						...
						if (matcher.find()){
							...
							if (alocacao.equals("1.000000")){
								...
								i=0;
								achouServer=false;
								while(i<EDGES.length&&!achouServer){
									...
									if (EDGES[i][0].equals(t2_server)){
										...
										fluxoGeradoAglomeradoVMs[Integer.parseInt(EDGES[i][1])]+=fluxoGeradoVM;
