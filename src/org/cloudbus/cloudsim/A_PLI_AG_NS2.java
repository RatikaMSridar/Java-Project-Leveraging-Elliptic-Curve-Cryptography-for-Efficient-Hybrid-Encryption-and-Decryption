...
public class A_PLI_AG_NS2 { 
	...
	//To indicate the next active server in datacenter
	private static int FF_SCALE_POINTER=0;
	...
	public A_PLI_AG_NS2(){  
		...
		if (FF){
			...
			//int VMsAlocadasAntes=0;
			//int VMsAlocadasDepois=1;//apenas para iniciar
			
			//-1 because index begin at 0
			FF_DATACENTER_POINTER=NUM_DATACENTERS-1;
			//-1 because index begin at 0
			FF_SERVER_POINTER=(NUM_DATACENTERS*NUM_SERVIDORES)-1;
			FF_SCALE_POINTER=0;
			
			//Apenas termina quando todos os grupos foram alocados e
			//foram percorridos todos os servidores na tentativa de alocar um grupo
			while(NUM_ITERACAO<=FF_GROUP && avoidLoop < S.length ){				
				
				//System.out.println("VMsAlocadasAntes: " + VMsAlocadasAntes + " VMsAlocadasDepois: " + VMsAlocadasDepois);
				...
				//Only update allocation with occurs allocation
				if (alocou){					
					...
					//Only write log if occurs allocation
					FF_writeLog();					
					
					/*VMsAlocadasAntes = VMsAlocadasDepois;
					VMsAlocadasDepois=0;
					p=0;
					while(p<S.length){			
						VMsAlocadasDepois += Integer.parseInt(S[p][I_SERVER_COST_CPU]);						
						//Next server
						p++;
					}//end while
					*/
					...
					System.out.println(
							"\nMainAntes: FF_SERVER_POINTER: " + FF_SERVER_POINTER + 
							" FF_SCALE_POINTER: " + FF_SCALE_POINTER +
							" FF_DATACENTER_POINTER: " + FF_DATACENTER_POINTER);
					
					avoidLoop++;
					//Apenas escala depois que todos os servidores foram vistos uma vez no laco
					if(avoidLoop==1)
						FF_SCALE_POINTER++;
					...
					//if(FF_DATACENTER_POINTER<0)
						//-1 because datacenter index begin at 0
						//FF_DATACENTER_POINTER=NUM_DATACENTERS-1;

					//? Qual eh o indice do ultimo servidor de cada datacenter?
					//+1 because begin at 0 index (Ex.: 0..3) 
					//-1 because begin at 0 index (Ex.: 0..9, 10..19, 20..29, ..)
					//Scale only when can not allocate					
					FF_SERVER_POINTER-=(NUM_SERVIDORES);
					if(FF_SERVER_POINTER<0){
						//-1 because index begin at 0
						FF_SERVER_POINTER=((NUM_DATACENTERS*NUM_SERVIDORES)-1)-FF_SCALE_POINTER;
						//When FF_SCALE_POINTER is the maximum value, FF_SERVER_POINTER can be negative
						if(FF_SERVER_POINTER<0)
							FF_SERVER_POINTER=0;
					}
					
					System.out.println(
							"\nMainDepois: FF_SERVER_POINTER: " + FF_SERVER_POINTER + 
							" FF_SCALE_POINTER: " + FF_SCALE_POINTER +
							" FF_DATACENTER_POINTER: " + FF_DATACENTER_POINTER);
	...
	public boolean FF_alocarGrupo(){
		...
		boolean allocated=true;
		//As FF_SERVER_POINTER can return to 0 index
		...
		while(i<C.length && allocated){
			...
			System.out.println("\n\nFF_GROUP: " + NUM_ITERACAO + " AlocarVM: " + i);
			q=0;
			...
			System.out.print("\n");
			q=0;
			while(q<S_CACHE.length){			
				//I_SERVER_COST_CPU is used to keeps the amount of VMs allocated 
				System.out.print(S_CACHE[q][I_SERVER_COST_CPU] + " ");
				q++;
			}//end while
			...
			while(!allocated && avoidLoop<S_CACHE.length ){
				...
				if(serverCPU >= vmCPU && serverBW >= vmBW){
					...
					//Go to the next server turned-on in the next datacenter

					System.out.println(
							"\n1Antes: " +
							"FF_SERVER_POINTER: " + FF_SERVER_POINTER +  
							"\nFF_DATACENTER_POINTER: " + FF_DATACENTER_POINTER +
							"\nFF_SCALE_POINTER: " + FF_SCALE_POINTER );					
					
					//FF_SCALE_POINTER indicate the current active server					
					FF_SERVER_POINTER-=(NUM_SERVIDORES);
					if(FF_SERVER_POINTER<0)
						//-1 because index begin at 0
						FF_SERVER_POINTER=((NUM_DATACENTERS*NUM_SERVIDORES)-1)-FF_SCALE_POINTER;
					...
					System.out.println(
							"1Depois: FF_SERVER_POINTER: " + FF_SERVER_POINTER + 
							"\nFF_DATACENTER_POINTER: " + FF_DATACENTER_POINTER +
							"\nFF_SCALE_POINTER: " + FF_SCALE_POINTER);					
					...
					//When FF_SCALE_POINTER is the maximum value
					//if(FF_SCALE_POINTER==NUM_VMS-1)
					//	FF_SCALE_POINTER=0;
					
					System.out.println(
							"\n--2Antes: FF_SERVER_POINTER: " + FF_SERVER_POINTER + 
							"\nFF_DATACENTER_POINTER: " + FF_DATACENTER_POINTER +
							"\nFF_SCALE_POINTER: " + FF_SCALE_POINTER);
					...
					System.out.println(
							"--2Depois: FF_SERVER_POINTER: " + FF_SERVER_POINTER + 							
							"\nFF_DATACENTER_POINTER: " + FF_DATACENTER_POINTER +
							"\nFF_SCALE_POINTER: " + FF_SCALE_POINTER);
	...
	public void FF_writeLog(){		
		...
		try {
			...
			out.write("FF_SERVER_POINTER: " + FF_SERVER_POINTER + " FF_DATACENTER_POINTER: " + FF_DATACENTER_POINTER + " FF_SCALE_POINTER: " + FF_SCALE_POINTER);
